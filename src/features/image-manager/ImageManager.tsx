    ...
    <Modal>
      <div className="p-4 w-11/12 lg:w-2/5 bg-white rounded relative">
        <div className="absolute flex justify-center right-4 top-4 z-1100">
          <IconButton
            text=""
            icon={<XIcon className="icon-size" />}
            onClick={() => dispatch(imageManagerActions.close())}
          />
        </div>
        ...
        {imageManager.startScreen && (
          <>
            ...
            <div className="mt-4 flex flex-col gap-y-2">
              <IconButton
                text={STRINGS.IMAGES_UPLOAD}
                icon={<UploadIcon className="icon-size" />}
                onClick={() => dispatch(imageManagerActions.selectImages())}
              />
              <IconButton
                text={STRINGS.IMAGES_MANAGE}
                icon={<PhotographIcon className="icon-size" />}
                onClick={() => dispatch(imageManagerActions.editImages())}
              />
            </div>
