...
import GeocodeWebEntity from '../../common/types/GeocodeWebEntity';
    ...
    getReverseGeocode: builder.query<GeocodeWebEntity, string>({
      ...
      transformResponse: (responseData: FeatureCollection) => {
        return responseData.features.map((geocodeJson: any) => {
          const address =
            geocodeJson.properties.Adresse +
            ', ' +
            geocodeJson.properties.PostalCode +
            ' ' +
            geocodeJson.properties.Municipality;
          return {
            address,
            coordinates: geocodeJson.geometry.coordinates,
          };
...
export const { useLazyGetGeocodeQuery, useLazyGetReverseGeocodeQuery } = geocodeApi;
