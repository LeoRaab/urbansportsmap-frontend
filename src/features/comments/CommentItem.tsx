      ...
      <div className="my-2">
        <Card>
          <p className="text-sm font-bold mb-4">
            {comment.author.name}, {commentDate}
          </p>
          ...
          {comment.author.id === userId && (
            <div className="flex justify-between border-t border-t-slate-200 pt-2 mt-4">
              <IconButton
                text={STRINGS.COMMENTS_DELETE}
                icon={<TrashIcon className="icon-size" />}
                onClick={handleDeleteCommentClick}
              />
              <IconButton
                text={STRINGS.COMMENTS_EDIT}
                icon={<PencilAltIcon className="icon-size" />}
